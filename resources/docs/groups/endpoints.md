...
curl -X POST \
    ...
    -d '{"name":"blanditiis","email":"pasquale.huel@example.org","password":"dolorem"}'
...
let body = {
    "name": "blanditiis",
    "email": "pasquale.huel@example.org",
    "password": "dolorem"
}
...
curl -X POST \
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"email":"kessler.sofia@example.com","password":"facere"}'
...
let body = {
    "email": "kessler.sofia@example.com",
    "password": "facere"
}
...
```bash
curl -X GET \
    -G "http://localhost:8000/api/v1/refresh" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
...
<form id="form-GETapi-v1-refresh" data-method="GET" data-path="api/v1/refresh" data-authed="1" data-hasfiles="0" data-headers='{"Content-Type":"application\/json","Accept":"application\/json"}' onsubmit="event.preventDefault(); executeTryOut('GETapi-v1-refresh', this);">
<h3>
    Request&nbsp;&nbsp;&nbsp;
...
<p>
<label id="auth-GETapi-v1-refresh" hidden>Authorization header: <b><code>Bearer </code></b><input type="text" name="Authorization" data-prefix="Bearer " data-endpoint="GETapi-v1-refresh" data-component="header"></label>
</p>
</form>
...
```bash
curl -X GET \
    -G "http://localhost:8000/api/v1/wallet/dolores/balance" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
...
```javascript
const url = new URL(
    "http://localhost:8000/api/v1/wallet/dolores/balance"
);
...
```json
{
    "message": "No query results for model [App\\Models\\Wallet] dolores",
...
```bash
curl -X GET \
    -G "http://localhost:8000/api/v1/wallet/ut/address" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
...
```javascript
const url = new URL(
    "http://localhost:8000/api/v1/wallet/ut/address"
);
...
```json
{
    "message": "No query results for model [App\\Models\\Wallet] ut",
    "exception": "Symfony\\Component\\HttpKernel\\Exception\\NotFoundHttpException",
    "file": "\/Users\/musti\/coinphon\/vendor\/laravel\/framework\/src\/Illuminate\/Foundation\/Exceptions\/Handler.php",
    "line": 368,
    "trace": [
